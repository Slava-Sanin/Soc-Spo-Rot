function WindowFunc(hwnd, message, wParam, lParam)
{
    ...
    switch (message)
    {
        case WM_CREATE:
            // Status bar creating.
                 if (glob_sound)
                 {
                     var myAudio = new Audio("G4W/sound/startgame.wav");
                     myAudio.play();
                 }
...
//          0, 0, 0, 0, hwnd, NULL, hInst, NULL);
        ...
        case WM_DESTROY: // Destroying Timer cicle and created objects.
            ...
            if (glob_sound)
                {
                    var myAudio = new Audio("G4W/sound/reminder.wav");
                    myAudio.play();
                }
            break;
        ...
        case WM_NOTIFY: // Change game.
            ...
            if (nmptr.code == TCN_SELCHANGE)
            {
                if (glob_sound)
                            {
                                var myAudio = new Audio("G4W/sound/changepage.wav");
                                myAudio.play();
                            }
                var tab;
...
function func1()
{
    ...
    if (p1 == NULL)
    {
        ...
        if (glob_sound)
                    {
                        var myAudio = new Audio("G4W/sound/load.wav");
                        myAudio.play();
                    }
        tci.pszText="Socoban";
        tci.lParam=1;
...
{
    if (!loaded) return;
    var tab=TabCtrl_GetCurSel(hTabWnd);
    if (glob_sound)
                {
                    var myAudio = new Audio("G4W/sound/down.wav");
                    myAudio.play();
                }
...
function write_config_file()
{
    struct
    {
        ...
        var table;
        st_Player pl,comp;
        var lpath0[255], lpath1[255], lpath2[255], lpath3[255];
    }temp;

    var handle;
    var length=sizeof(temp);
    var filename[255];
...
BOOL CALLBACK BkgrDlgProc (HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    static var i;
    static var str_drive[6]="";
    static var str_directory[255]="";
    static var str_fullpath[255]="";
    static var str_file[255]="";
    static var who_for; //Who to choose background for
...
//////////////////////////////////////////////////////////////////////////
function drawString(HWND hwndWhere, var *string, var color)
{
    static HDC hdc;
...
function makeBackGround(HWND hwnd, whatever *P_this, var *str)
{
